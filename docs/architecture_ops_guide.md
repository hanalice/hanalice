...
- **写入 `main`**：ruleset 禁止 `GITHUB_TOKEN` 直推默认分支。流水线用 secret `TRAFFIC_PAT` 开 `chore/traffic-rollup` 的 PR 并 squash 合并。PAT 除 Administration: Read 外还需 **Contents: Write** 与 **Pull requests: Write**。
