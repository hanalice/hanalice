...
def _toc_display_label(item):
    """Build TOC label without duplicating existing numbering / em-dash prefixes."""
    text = item['text']
    if item['level'] == 2:
        if _NUMBERED_HEADING_RE.match(text):
            return text
        return f'{item["number"]}. {text}'
    # H3: keep — prefix without duplicating
    if _EMDASH_PREFIX_RE.match(text):
        return text
    return f'— {text}'
...
def _toc_html(toc_items):
    ...
    for item in toc_items:
        ...
        display = html.escape(_toc_display_label(item))
...
def _page_shell(
    ...
    parts = [
        ...
        f'<main class="{html.escape(main_class, quote=True)}">',
        body_html,
        '</main>',
...
def _render_post(post):
    ...
    if toc:
        ...
        # Article first in DOM (reading order); CSS grid places TOC on the right.
    ...
    return _page_shell(
        ...
        body_class='has-toc-layout' if toc else '',
    )
